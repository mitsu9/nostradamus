...
require_relative 'lib/nostradamus'

Dotenv.load

class NostradamusBot < SlackRubyBot::Bot
  def initialize
    @flag_int = false
    @pid_file = "./nostradamus-bot-daemon.pid"
  end
  ...
  def self.logger
    @@logger ||= Logger.new(File.expand_path(File.dirname(__FILE__) + "/log/nostradamus-bot.log"), "weekly")
  end
end


begin
  NostradamusBot.new.run
rescue Exception => e
  NostradamusBot.logger.error "#{e.inspect} backtrace: #{e.backtrace}"
  raise e
end
